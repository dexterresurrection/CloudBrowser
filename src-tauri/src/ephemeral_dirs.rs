...
fn get_or_create_macos_ramdisk() -> Result<PathBuf, String> {
  let mount_point = PathBuf::from("/Volumes/CloudBrowserEphemeral");
  ...
  let fmt = std::process::Command::new("diskutil")
    .args(["erasevolume", "HFS+", "CloudBrowserEphemeral", &dev])
    .output()
...
fn get_or_create_windows_ramdisk() -> Result<PathBuf, String> {
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    let base = PathBuf::from(format!("{}:\\CloudBrowserEphemeral", letter));
  ...
  for letter in ['R', 'Q', 'P', 'O'] {
    ...
    match output {
      Ok(out) if out.status.success() => {
        let base = PathBuf::from(format!("{}\\CloudBrowserEphemeral", drive));
        std::fs::create_dir_all(&base)
