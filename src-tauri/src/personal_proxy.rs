...
pub async fn personal_proxy_get_list() -> Result<Vec<PersonalProxyConfig>, String> {
    ...
    let url = format!("{}/donut/proxies/{}", api_url.trim_end_matches('/'), telegram_id);
    let body = api_get(&url, &api_key).await?;
    serde_json::from_str(&body).map_err(|e| format!("Parse error: {e}"))
}
...
pub async fn personal_proxy_check_subscription() -> Result<PersonalSubscriptionInfo, String> {
    let (api_url, api_key, telegram_id) = get_api_settings()?;
    let url = format!("{}/donut/subscription/{}", api_url.trim_end_matches('/'), telegram_id);
    let body = api_get(&url, &api_key).await?;
    serde_json::from_str(&body).map_err(|e| format!("Parse error: {e}"))
}
...
pub async fn personal_proxy_create(
    ...
    // Проверить подписку
    let sub_url = format!("{}/donut/subscription/{}", api_url.trim_end_matches('/'), telegram_id);
    ...
    let create_url = format!("{}/donut/proxies/{}", api_url.trim_end_matches('/'), telegram_id);
    let body = api_post(
        &create_url,
        &api_key,
    ...
    Ok(format!("Proxy '{}' created successfully", config.display_name))
}

#[tauri::command]
pub async fn personal_proxy_delete(
    config_id: i64,
) -> Result<(), String> {
    let (api_url, api_key, telegram_id) = get_api_settings()?;
    let url = format!(
