...
mod macos {
  ...
  pub fn is_default_browser() -> Result<bool, String> {
    ...
    let bundle_id = "com.cloudbrowser";
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    let bundle_id = CFString::new("com.cloudbrowser");
    let schemes = ["http", "https"];
      ...
      unsafe {
        ...
        if status != 0 {
          let error_msg = match status {
            -54 => format!(
              "Failed to set as default browser for scheme '{scheme}'. The app is not properly registered as a browser. Please:\n1. Build and install the app properly\n2. Manually set Cloud Browser as default in System Settings > General > Default web browser\n3. Make sure the app is in your Applications folder"
            ),
            _ => format!(
              "Failed to set as default browser for scheme '{scheme}'. Status code: {status}. Please manually set Cloud Browser as default in System Settings > General > Default web browser."
            )
          };
...
mod windows {
  ...
  fn register_application(exe_path: &str) -> Result<(), String> {
    ...
    app_key
      .set_value(
        ...
        &"Cloud Browser - Simple Yet Powerful Anti-Detect Browser",
      )
  ...
  fn register_prog_id(exe_path: &str) -> Result<(), String> {
    ...
    prog_id_key
      .set_value("", &"Cloud Browser Document")
      .map_err(|e| format!("Failed to set ProgID default value: {}", e))?;

    prog_id_key
      .set_value("FriendlyTypeName", &"Cloud Browser Document")
...
mod linux {
  ...
  pub fn set_as_default_browser() -> Result<(), String> {
    ...
    if !check_desktop_file_exists() {
      return Err(format!(
        "Desktop file '{}' not found in standard locations. Please ensure the application is properly installed. You can manually set Cloud Browser as the default browser in your system settings.",
        APP_DESKTOP_NAME
      ));
    }
