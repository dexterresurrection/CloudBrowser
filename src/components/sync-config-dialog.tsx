...
export function SyncConfigDialog({ isOpen, onClose }: SyncConfigDialogProps) {
  ...
  // Personal proxy state
  const [ppApiUrl, setPpApiUrl] = useState("");
  ...
  >("unknown");
  // Load personal proxy settings on open
  useEffect(() => {
    if (!isOpen) return;
    void (async () => {
      setPpLoading(true);
      try {
        ...
        }>("personal_proxy_get_settings");
        setPpApiUrl(s.api_url ?? "");
        setPpApiKey(s.api_key ?? "");
        setPpTelegramId(s.telegram_id ? String(s.telegram_id) : "");
      } catch {
        // not configured yet
      } finally {
        setPpLoading(false);
        ...
        {isLoggedIn && user ? (
          <div className="grid gap-4 py-4">
            ...
            <div className="flex gap-2 pt-2">
              <Button variant="outline" className="flex-1" asChild>
                <a
                  href="https://cloudbrowser.chocobo.fun/account"
                  target="_blank"
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="w-full">
              ...
              <TabsTrigger value="personal-proxy" className="flex-1">
                Personal Proxy
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="personal-proxy">
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  <Label>API URL</Label>
                  <Input
                    placeholder="https://apibot.example.com"
                    value={ppApiUrl}
                    onChange={(e) => setPpApiUrl(e.target.value)}
                  />
                </div>
                <div className="grid gap-2">
                  <Label>API Key</Label>
                  <div className="relative">
                    <Input
                      type={showPpApiKey ? "text" : "password"}
                      placeholder="Your API key"
                      value={ppApiKey}
                      onChange={(e) => setPpApiKey(e.target.value)}
                      className="pr-10"
                    />
                    <button
                      type="button"
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    value={ppTelegramId}
                    onChange={(e) => setPpTelegramId(e.target.value)}
                  />
                </div>
              </div>
              <DialogFooter>
                <LoadingButton
                  isLoading={ppSaving}
                  disabled={!ppApiUrl || !ppApiKey || !ppTelegramId}
                  onClick={async () => {
                    setPpSaving(true);
                    try {
