...
export function ProfilesDataTable({
  ...
  const loadPpServers = React.useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load personal proxy servers:", e);
    }
  }, [ppServersLoaded]);

  const handleCreatePersonalProxy = React.useCallback(
    async (profileId: string, serverId: number, serverName: string) => {
      ...
      try {
        ...
        const proxyName = `Personal: ${serverName} ${Date.now()}`;
        await invoke("personal_proxy_create", {
          appHandle: null,
          serverId,
        ...
        showSuccessToast("Personal proxy created");
      } catch (e) {
                  ...
                  >
                    <Command>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup heading="Personal Proxy">
                          <CommandItem
                            value="__load-personal__"
                          ...
                          {meta.ppServers.map((srv) => (
                            <CommandItem
                              key={`pp-${srv.id}`}
                              value={`personal-${srv.name}`}
                              onSelect={() =>
                                void meta.handleCreatePersonalProxy(
                                  profile.id,
                                  srv.id,
                                  srv.name,
                                )
